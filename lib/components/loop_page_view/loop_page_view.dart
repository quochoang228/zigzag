...
import 'loop_page_controller.dart';
...
class _LoopPageViewState extends State<LoopPageView> {
        ...
        ? NotificationListener<ScrollNotification>(
            onNotification: (scrollNotification) {
              if (scrollNotification is ScrollEndNotification) {
                ...
                _controller.updateCurrentShiftedPage();
              }
              return false;
            },
            child: PageView.builder(
              ...
              onPageChanged: (int index) {
                if (_controller.isAnimatingJumpToPage != true &&
                    widget.onPageChanged != null) {
                  widget.onPageChanged!(_controller.notShiftedIndex(index));
                }
              },
              itemBuilder: (context, index) {
                final int notShiftedIndex = _controller.notShiftedIndex(index);

                if (_controller.isAnimatingJumpToPage == true &&
                    currentPage != null &&
                    notShiftedIndex == _controller.isAnimatingJumpToPageIndex) {
                  _controller.isAnimatingJumpToPage = false;
                  return currentPage ?? const SizedBox.shrink();
                }
  ...
  void didUpdateWidget(LoopPageView oldWidget) {
    if (oldWidget.itemCount != widget.itemCount) {
      _controller.updateItemCount(widget.itemCount);
    }
